...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        proxy.setHostName("4.16.68.158")
        proxy.setPort(443)
        proxy.setUser("cat_surf")
        proxy.setPassword("SurfCatPssword")
        ...
        self.browser.page().fullScreenRequested.connect(
            lambda request, browser=self.browser: self.handleFullscreenRequest(
                request, browser
            )
        )

        # self.browser.loadProgress.connect(self.webLoading)
        self.browser.loadStarted.connect(self.download_started)
        ...
        self.navtb.addAction(more_opt)
 
        downloadV = QAction("Videó letöltése", self)
        downloadV.triggered.connect(self.download_video_thread)
        downloadV.setStatusTip('Ezzel a funkcióval letölthetsz videókat a YouTube-ról, Shortcut: Ctrl + Shift + V')
    ...
    def on_downloadRequested(self, download):
        try:
            ...
            path, _ = QtWidgets.QFileDialog.getSaveFileName(
                self, "Fájl mentése", old_path, "*.*" + suffix
            )
            if path:
                print(suffix)
        ...
        # Notifikáció
        try:
            download.finished.connect(lambda:self.download_sucsessful(path.rsplit('/')[-1]))
        except:
            self.download_unsucsessful(path.rsplit('/')[-1])

    def download_sucsessful(self, filename):
        if filename != "":
            send_notification("Művelet sikeres!", f"A {filename} nevű fájllal végzett művelet sikeres.")

    def download_unsucsessful(self, filename):
        if filename != "":
            send_notification("Művelet sikertelen.", f"A {filename} nevű fájllal végzett művelet sikertelen.")

    def download_started(self, filename):
        if filename != "":  
            send_notification("A művelet elindult.", f"A {filename} nevű fájllal végzett művelet elkezdődött.")

    def handleFullscreenRequest(self, request, browser):
        request.accept()
    ...
    def update_title(self, browser):
        if self.urlbar.text() == "about:blank":
            self.navigate_home()
...
send_notification("A Cat Surf elindult.", "Sikeres indítás")
